...
int compareByMarkReverse(struct stab* i1, struct stab* i2) {
    if (strcmp(i1->mark, i2->mark) > 0) {
        return -1;
    }
    else if (strcmp(i1->mark, i2->mark) < 0) {
        return 1;
    }
    else {
        return 0;
...
void mergeSortStr(struct stab** headRef, int* kolvo, int dir) {
    ...
    mergeSortStr(&a, kolvo, dir);
    mergeSortStr(&b, kolvo, dir);

    *headRef = merge2(a, b, kolvo, dir);
}
...
struct stab* merge2(struct stab* a, struct stab* b, int* kolvo, int dir) {
    ...
    if (dir == 1) {
        if (compareByMarkReverse(a, b) <= 0) {
            result = a;
            result->next = merge2(a->next, b, kolvo, dir);
        }
        else {
            result = b;
            result->next = merge2(a, b->next, kolvo, dir);
        }
    }
    else {
        if (compareByMark(a, b) <= 0) {
            result = a;
            result->next = merge2(a->next, b, kolvo, dir);
        }
        else {
